...
# Wymagane zmienne z .env (docker-compose je wstrzyknie do kontenera db)
...
DO \$\$
BEGIN
   IF NOT EXISTS (SELECT 1 FROM pg_database WHERE datname = '${USER_DB_NAME}') THEN
      EXECUTE format('CREATE DATABASE %I OWNER %I', '${USER_DB_NAME}', '${USER_DB_USER}');
   END IF;

   IF NOT EXISTS (SELECT 1 FROM pg_database WHERE datname = '${CATALOG_DB_NAME}') THEN
      EXECUTE format('CREATE DATABASE %I OWNER %I', '${CATALOG_DB_NAME}', '${CATALOG_DB_USER}');
   END IF;
END
\$\$;
EOSQL
